  ...
  "criteriaSatisfied": [
    {
      "id": "criterion-1",
      "status": "satisfied",
      "evidence": "Concrete severity-tagged findings cite frontend source paths/lines, live OpenAPI evidence, and validation output in sections 7-18."
    }
  ],
  "changedFiles": [],
  "testsAddedOrUpdated": [],
  "commandsRun": [
    {"command":"git status --short --branch; git diff --check","result":"passed","summary":"No staged files; only pre-existing harness artifacts were untracked."},
    {"command":"curl http://localhost:5185/swagger/v1/swagger.json","result":"passed","summary":"HTTP 200; parsed live OpenAPI bearer/auth/contest contracts."},
    {"command":"npm run format:check","result":"failed","summary":"26 files require Prettier formatting."},
    {"command":"npm run lint","result":"passed","summary":"oxlint exit 0."},
    {"command":"npm run typecheck","result":"failed","summary":"TS5101 at tsconfig.app.json:8."},
    {"command":"npm run test:run","result":"failed","summary":"5 failed, 60 passed tests."},
    {"command":"npm run build","result":"failed","summary":"Blocked by TS5101 typecheck error."},
    {"command":"npm run dev (brief start/stop)","result":"failed","summary":"Vite announced ready; local curl returned 000, so serving could not be verified."}
  ],
  "validationOutput": [
    "Live OpenAPI HTTP 200", "lint passed", "format/typecheck/test/build failed as detailed in section 15"
  ],
  "residualRisks": [
    "Bearer session is not wired into HttpClient", "Admin/list feature is unreachable and incomplete", "Real backend response/roles and authenticated E2E behavior are not verified"
  ],
  "noStagedFiles": true,
  "diffSummary": "Read-only audit; no product-source changes. Required audit artifact only.",
  "reviewFindings": [
    "P0: src/lib/api/http-client.ts and src/features/auth/Pages/LoginPage.tsx - session token is never configured as shared Bearer authorization.",
    "P0: frontend/tsconfig.app.json:8 - TS5101 blocks typecheck/build.",
    "P1: src/features/contests/hooks.ts:1-4 and src/routes/router.tsx:32 - admin feature imports missing symbols and is not routed.",
    "P1: src/features/contests/types.ts:20-23 - create response is not supported by current live OpenAPI."
  ],
  "manualNotes": "No destructive commands, OpenSpec CLI, API mutations, or product file edits were performed."
}
```
